...
class GMVAE(nn.Module):
    ...
    def ELBO(self, X):
        ...
        h_sample = h_sample.reshape(self.args.M * self.args.batch_size, self.args.h_dim)
        recon_x =self.P(h_sample)  # [n_classes, M * bs, channel, image_size, image_size]
        recon_x = recon_x.reshape(self.args.n_classes, self.args.M, -1, self.args.channels, self.args.image_size, self.args.image_size)
    ...
    def recon_loss(self, recon_x, X, c_probs):
        bceloss = nn.BCELoss(reduction = 'none')(input = recon_x,  target=X.expand_as(recon_x))
        bceloss = torch.sum(bceloss, axis = [-1,-2,-3]) # [n_classes, M, bs]
        # c_probs.reshape(self.args.M * self.args.batch_size, self.args.n_classes)
        bceloss = bceloss.permute(1,2,0) 
